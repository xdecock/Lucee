...
public final class InterruptHandlerInjector {
	...
	public static int writeLoopInit(GeneratorAdapter adapter) throws TransformerException {
	...
	public static writeLoopBodyEnd(GeneratorAdapter adapter, int iteratorRef, Label jumpLabel, String timeoutDescription) {
	...
	public static writeEndPreempt(GeneratorAdapter adapter, String timeoutDescription) {
		Label endPreempt = new Label();
		writePreempt(adapter, label, timeoutDescription);
		adapter.visitLabel(label);
	}

	public static writePreempt(GeneratorAdapter adapter, Label label, String timeoutDescription) {
		// Check if the thread is interrupted
